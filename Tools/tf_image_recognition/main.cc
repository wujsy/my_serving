...
Status LoadGraph_new(string graph_file_name, char *temp,
                 std::unique_ptr<tensorflow::Session>* session) {
  tensorflow::GraphDef graph_def;
  Status load_graph_status =
      ReadBinaryProtoC(tensorflow::Env::Default(), temp, &graph_def);
  if (!load_graph_status.ok()) {
    return tensorflow::errors::NotFound("Failed to load compute graph at '",
                                        graph_file_name, "'");
  }
...
int main(int argc, char* argv[]) {
  ...
  string graph =
      "/data/wujiaxing/mydata/my_frozen_graph/v4_zh/inception_v4_zh_frozen.pb";
  //string graph = "tensorflow/examples/label_image/data/classify_image_graph_def.pb";
  string labels =
      "/data/wujiaxing/mydata/my_frozen_graph/v4_zh/auto_metadata.txt";
...
/*
//--------------------------------------------------------------------------------
  //int maxlen = 176*1024*1024;
  //char * tmp = new char[176*1024*1024];
  //memset(tmp, 0, 176*1024*1024);
  //char *key = "wujxtest";
  //DES_Decrypt(graph_path.c_str(), key, tmp);
  FILE *infile, *pFile;
  infile = fopen(graph_path.c_str(), "rb");
  if(infile == NULL)
    printf("1 open pb file failed!\n");
  ...
  pFile = fopen(graph_path.c_str(), "rb");
   if(pFile == NULL)
     printf("2 open pb file failed!\n");
  int rc = fread(temp, sizeof(char), size, pFile);
  printf("------ rc = %d\n", rc);
  fclose(pFile); 


  Status load_graph_status = LoadGraph_new(graph_path, temp, &session);
  if (!load_graph_status.ok()) {
    LOG(ERROR) << load_graph_status;
    return -1;
  }
  else
    printf("------ Load graph and variables done.\n");
//------------------------------------------------------------------------------
*/
  ...
  cout << "------Image Recognition done." << endl;
